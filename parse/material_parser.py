...
class MaterialParser:
    def __init__(
        ...
        self.__ions = json.loads(
            open(
                os.path.join(
                    self.__filename,
                    'rsc/ions_dictionary.json')).read())
        self.__anions = {
            ion['c_name']: {
                'valency': ion['valency'],
                'e_name': ion['e_name'],
                'n_atoms': ion['n_atoms']} for ion in self.__ions['anions']}
        self.__cations = {
            ion['c_name']: {
                'valency': ion['valency'],
                'e_name': ion['e_name'],
                'n_atoms': ion['n_atoms']} for ion in self.__ions['cations']}
        ...
        self.__pubchem_dictionary = json.loads(
            open(
                os.path.join(
                    self.__filename,
                    'rsc/pubchem_dict.json'),
                'rb').read())
        self.__abbreviations = json.loads(
            open(
                os.path.join(
                    self.__filename,
                    'rsc/abbreviations.json'),
                'rb').read())

        self.__fails_log = fails_log
        if fails_log:
            self.__pubchem_file = open(
                os.path.join(
                    self.__filename,
                    'pubchem_log'),
                'w')
    ...
    def parse_material(self, material_string_):
        ...
        material_name, material_formula, material_structure = self.split_material_name(
            material_string)
        ...
        if material_structure['elements'] != {}:
            ...
            output_structure['material_formula'] = self.__combine_formula(
        ...
        if all(part[1:].islower() for part in re.sub(
                r'[IV\(\)]', '', material_name).split(' ') if part[1:] != ''):
        ...
        for compound, amount in material_parts:
            try:
                ...
                output_structure['amounts_vars'].update(
                    structure['amounts_vars'])
                output_structure['elements_vars'].update(
                    structure['elements_vars'])
                if compound == 'H2O':
                    hydrate = amount
        ...
        # substituting additive into composition if it makes fractions to
        # sum-up to integer
        output_structure['additives'] = [
            elem.strip(' ') for additive in output_structure['additives'] for elem in re.split(
                r'[\s,]', additive) if elem != '']
        additive = additives[0].strip(' ') if len(additives) == 1 else ''
        #print('-->', additive)
        if additive != '' and all(c['elements'] != {}
                                  for c in output_structure['composition']):
            formula, composition = self.__substitute_additive(
                additive, material_formula, output_structure['composition'])
        ...
        try:
            if any(
                    float(s) < 0.0 for compound in output_structure['composition'] for e,
                    s in compound['elements'].items()):
                output_structure['composition'] = []
        except BaseException:
            pass
        ...
        output_structure['is_abbreviation_like'] = self.__is_abbreviation_like(
            output_structure)
    ...
    def get_structure_by_formula(self, formula):
        ...
        if formula[0].islower():
            for m in re.finditer(
                    '([' + ''.join(self.__greek_letters) + ']*)-{0,1}(.*)', formula):
                if m.group(2) != '':
                    phase = m.group(1)
                    formula = m.group(2)
        ...
        hydrate = ''
        if 'H2O' in formula and any(
            c in formula for c in [
                '·', '•', '-', '×', '⋅']):
            formula = formula[1:-
                              1] if formula[0] == '(' and formula[-1] == ')' else formula
        ...
        for m in re.finditer(r, formula):
            if ',' in m.group(1):
                ...
                formula = formula.replace(
                    '(' + m.group(1) + ')' + m.group(2), 'M' + m.group(2), 1)
            if not m.group(1).isupper() and m.group(2) == '':
                formula = formula.replace(
                    '(' + m.group(1) + ')', m.group(1), 1)
        ...
        formula_structure = dict(elements=composition,  # {e: s for e, s in composition.items()},
                                 amounts_vars={
                                     x: v for x, v in stoichiometry_variables.items()},
                                 elements_vars={
                                     e: v for e, v in elements_variables.items()},
                                 hydrate=hydrate,
                                 phase=phase,
                                 formula=formula,
    ...
    def __parse_formula(self, init_formula):
        ...
        formula_dict = self.__parse_parentheses(
            init_formula, "1", formula_dict)
    ...
    def __parse_parentheses(self, init_formula, init_factor, curr_dict):
        ...
        for m in re.finditer(r, init_formula):
            ...
            factor = self.__simplify(
                '(' + str(init_factor) + ')*(' + str(factor) + ')')

            unit_sym_dict = self.__parse_parentheses(
                m.group(1), factor, curr_dict)
    ...
    def __get_sym_dict(self, f, factor):
        ...
        r = r"([A-Z□]{1}[a-z]{0,1})\s*([-\*\.\da-z" + \
            ''.join(self.__greek_letters) + r"\+\/]*)"
    ...
    def split_material_name(self, material_string):
        ...
        if composition_e != {} and formula_e not in self.__list_of_elements_1 + \
                self.__list_of_elements_2:
            split = split[:-1]
        ...
        name_terms = [p for p in split if p.lower().strip(
            '., -;:').rstrip('s') in self.__chemicals or 'hydrate' in p]
    ...
    def reconstruct_formula(self, material_name, valency=''):
        ...
        _, anion_prefix_num, anion = ('', 0, anion) if anion.lower(
        ) in self.__anions else self.__get_prefix(anion)
        ...
        if len(terms_list) == 1:
            ...
            _, cation_prefix_num, cation = ('', 0, cation) if cation.lower(
            ) in self.__cations else self.__get_prefix(cation)
        ...
        output_formula = self.__build_formula(
            anion=anion_data,
            cation=cation_data,
        ...
        if hydrate != '':
            ...
            hydrate_prefix = '' if hydrate_prefix_num in [
                0, 1] else str(hydrate_prefix_num)
    ...
    def __build_formula(
        ...
        if anion_prefix_num + cation_prefix_num == 0 or anion_prefix_num * \
                cation_prefix_num != 0:
        ...
        if anion_prefix_num != 0:
            ...
            while cation_stoich == 0 and i < len(cation['valency']):
                cation_stoich = anion_prefix_num * \
                    abs(anion['valency'][0]) // abs(cation['valency'][i])
                i = i + 1

        if cation_prefix_num != 0:
            cation_stoich = cation_prefix_num
            anion_stoich = cation_prefix_num * \
                abs(cation['valency'][0]) // abs(anion['valency'][0])
        ...
        return "{0}{1}{2}{3}".format(cation_name_el, self.__cast_stoichiometry(
            cation_stoich), anion_name_el, self.__cast_stoichiometry(anion_stoich))
    ...
    def __split_name(self, material_name_, init_fraction='1'):
        ...
        re_str = "(?<=[0-9\)])[-⋅·∙\∗](?=[\(0-9])|(?<=[A-Z])[-⋅·∙\∗](?=[\(0-9])|(?<=[A-Z\)])[-⋅·∙\∗](?=[A-Z])|(?<=[" \
                 r"0-9\)])[-⋅·∙\∗](?=[A-Z])"
        re_str = re_str + \
            ''.join(['|(?<=' + e + r')[-⋅·∙\∗](?=[\(0-9A-Z])' for e in self.__list_of_elements_1 + self.__list_of_elements_2])
        ...
        if len(parts) > 1:
            parts_upd = [p for part in parts for p in re.split(
                '(?<=[A-Z\)])[-·∙\∗⋅](?=[xyz])|(?<=O[0-9\)]+)[-·∙\∗⋅](?=[xyz])', part)]
        else:
            parts_upd = parts

        #print ('-->', parts_upd)

        if any(m.strip('0987654321') in self.__list_of_elements_1 +
               self.__list_of_elements_2 for m in parts_upd[:-1]):
    ...
    def get_additives(self, material_name):
        ...
        for part_ in new_material_name.split(':'):
            ...
            if all(
                e.strip('zyx,+0987654321. ') in self.__list_of_elements_1 +
                ['R'] +
                ['&&'] for e in re.split(
                    r'[\s,/]',
                    part) if e != ''):
    ...
    def build_abbreviations_dict(self, materials_list, paragraph):
        ...
        abbreviations_dict = {
            t: '' for t in materials_list if self.__is_abbreviation(
                t.replace(
                    ' ', '')) and t != ''}
        ...
        for abbr, name in abbreviations_dict.items():
            sents = ' '.join(
                [s + ' ' for s in paragraph if abbr in s]).strip(' ').split(abbr)
            i = 0
        ...
        for abbr in abbreviations_dict.keys():
            parts = re.split('-', abbr)
            if all(p in abbreviations_dict for p in parts) and abbreviations_dict[abbr] == '' and len(
                    parts) > 1:
                name = ''.join(
                    '(' + abbreviations_dict[p] + ')' + '-' for p in parts).rstrip('-')
                abbreviations_dict[abbr] = name

        empty_list = [
            abbr for abbr,
            name in abbreviations_dict.items() if name == '']
    ...
    def __get_values(self, string, mode):
        ...
        if mode == 'range':
            ...
            try:
                ...
                min_value = round(
                    float(
                        smp.simplify(min_value)),
                    4) if min_value != '' else None
        ...
        if mode == 'values':
            ...
            try:
                values = [round(float(smp.simplify(c.rstrip('., '))), 4)
                          for c in values if c.rstrip('., ') not in ['', 'and']]
    ...
    def get_stoichiometric_values(self, var, sentence):
        ...
        regs = [
            (var + '\s*=\s*([-]{0,1}[0-9\.\,/and\s]+)[\s\)\]\,]',
             'values'),
            (var + r'\s*=\s*([0-9\.]+)\s*[-–]\s*([0-9\.\s]+)[\s\)\]\,m\%]',
             'range'),
            (r'([0-9\.\s]*)\s*[<≤⩽]{0,1}\s*' + var + r'\s*[<≤⩽>]{1}\s*([0-9\.\s]+)[\s\)\]\.\,]',
             'range'),
            (var + r'[a-z\s]*from\s([0-9\./]+)\sto\s([0-9\./]+)',
             'range'),
        ]
    ...
    def get_elements_values(self, var, sentence):
        ...
        values = re.findall(
            ...
            sentence)
        values = [
            c.rstrip('0987654321+') for v in values for c in re.split(
                r'[,\s]',
                v) if c.rstrip('0987654321+') in self.__list_of_elements_1 +
            self.__list_of_elements_2]
    ...
    def is_materials_list(self, material_string):
        if (any(a + 's' in material_string.lower() for a in self.__anions.keys())
                or 'metal' in material_string) and any(w in material_string for w in ['and ', ',', ' of ']):
            return True
    ...
    def reconstruct_list_of_materials(self, material_string):
       ...
       #print (self.__anions.keys())
        ...
        anion = [(i, p[:-1]) for i, p in enumerate(parts) if p[:-1].lower()
                 in self.__anions.keys() or p[:-1].lower() == 'metal']
        ...
        valencies = [(i - 1, p.strip('()')) for i, p in enumerate(parts)
                     if p.strip('()') in self.__rome2num and i != 0]
       ...
       # print ('->', anion, cation)
        ...
        if len(anion) == 1:
            for c_i, c in cation:
                ...
                result.append(
                    (''.join([n + ' ' for n in name]).strip(' '), valency))
    ...
    def cleanup_name(self, material_name):
        ...
        material_name = re.sub(r'\s*([-+±]){1}\s*([' +
                               ''.join([c for c in self.__greek_letters]) +
                               ']{1})', '\\1δ', material_name)
        ...
        material_name = re.sub(
            r'\({0,1}[0-9\.]*\s*[⩽≤<]{0,1}\s*[x,y]{0,1}\s*[⩽=≤<]\s*[0-9\.-]*\){0,1}',
            '',
            material_name)

        if material_name == '' or len(
                [c for c in material_name if c.isalpha()]) < 1:
            return ''
        ...
        for c in [
            ...
            if len(split) > 1 and (
                    len(split[-1]) == '' or all(not s.isalpha() for s in split[-1])):
    ...
    def __check_parentheses(self, formula):
        ...
        if new_formula[0] == '(' and new_formula[-1] == ')' and len(
                par_close) == 1 and len(par_open) == 1:
    ...
    def __simplify(self, value):
        ...
        for i, m in enumerate(re.finditer(
                '(?<=[0-9])([a-z' + ''.join(self.__greek_letters) + '])', new_value)):
            new_value = new_value[0:m.start(
                1) + i] + '*' + new_value[m.start(1) + i:]
    ...
    def __substitute_additive(
        ...
        for compound in material_composition:
            expr = ''.join(['(' + v + ')+' for e,
                            v in compound['elements'].items()]).rstrip('+')

            coeff = coeff[0] if not re.match(
                '^[0]+[1-9]', coeff[0]) else '0.' + coeff[0][1:]
            expr = expr + '+(' + coeff + ')'
            ...
            if self.__is_int(self.__simplify(expr)):
                ...
                new_material_formula = new_material_formula.replace(
                    compound['formula'], new_name)
            else:
                new_material_composition.append(dict(
    ...
    def __is_abbreviation_like(self, structure):
        if any(all(e.isupper() and s in ['1.0', '1'] for e, s in compound['elements'].items(
        )) for compound in structure['composition']):
            # if all(e.isupper() and s in ['1.0', '1'] for f, c in
            # m_struct['composition'].items() for e, s in c.items()):
            return True

        elements_vars = [el for el in structure['elements_vars'].keys() if len(
            el) == 1 and el.isupper()]
        ...
        if re.findall(
            '[A-Z]{3,}',
            structure['material_formula']) != [] and all(
            w not in structure['material_formula'] for w in [
                'CH',
                'COO',
                'OH']):
            return True
